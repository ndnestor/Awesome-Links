...
var methods = {
    ...
    cacheRecords: function (tableName) {
        global_logger_1.methods.debug("Caching records for \"" + tableName + "\"");
        return new Promise(function (resolve, reject) {
            handleDbRequest(function () {
                ...
                airtableBase(tableName).select({
                    ...
                    global_logger_1.methods.error("Could not update cached records due to error\n" + error);
                });
            });
        });
    },
    ...
    getCachedRecords: function (tableName) {
        ...
        if (result === undefined) {
            global_logger_1.methods.warn("Table \"" + tableName + "\" has no cached records");
        }
        return result;
    },
    ...
    searchInField: function (tableName, fieldName, fieldValue, isExact) {
        global_logger_1.methods.info("Searching for record by field value. " +
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (cachedRecords[tableName] !== undefined) {
                ...
                global_logger_1.methods.warn('Trying to search in a table that is not cached (and may not exist)');
                reject();
            }
    ...
    addRecords: function (tableName, records) {
        global_logger_1.methods.info("Adding record to \"" + tableName + "\"");
        return new Promise(function (resolve, reject) {
            handleDbRequest(function () {
                airtableBase(tableName).create(records, { typecast: true }).then(function (addedRecords) {
                    resolve(addedRecords);
                }).catch(function (error) {
                    global_logger_1.methods.error("Could not add record to table \"" + tableName + "\"\n" + error);
                    reject();
                });
            });
        });
    ...
    deleteRecords: function (tableName, recordIDs) {
        global_logger_1.methods.info("Deleting record(s) from \"" + tableName + "\" with ID(s) \"" + recordIDs + "\"");
        return new Promise(function (resolve, reject) {
            handleDbRequest(function () {
                airtableBase(tableName).destroy(recordIDs).then(function (deletedRecords) {
                    resolve(deletedRecords);
                }).catch(function (error) {
                    global_logger_1.methods.error("Could not delete record(s) from \"" + tableName + "\" with ID(s) \"" + recordIDs + "\"\n" + error);
                    reject();
                });
            });
        });
...
function handleDbRequest(method) {
    ...
    if (timeSinceLastDbRequest < (1 / MAX_DB_REQUESTS_PER_SECOND) * 1000) {
        ...
        global_logger_1.methods.info('Deferring database request to prevent overload');
